...
insert into Sales (ClientId, StorageId, [Date], Quantity, EmployeeId) values (54, 14, '2018-12-16',  6, 17);
 ...
 --------------------------------------------------------------------------------------------------------------------
 --------------------------------------------------------------------------------------------------------------------
-- Задание. Для базы данных «Спортивный магазин» из практического задания модуля «Триггеры, хранимые процедуры и пользовательские функции» 
...
RETURNS INT
BEGIN
 DECLARE @uniqueClietn INT = (SELECT DISTINCT COUNT(*) AS 'Unique Client'  FROM Clients)
 RETURN @uniqueClietn
END

SELECT  dbo.UniqClient() AS 'Unique Client'
...
CREATE FUNCTION AvgByType(@type NVARCHAR(30))
RETURNS INT
BEGIN
		DECLARE @avgByTypeResult INT = (SELECT AVG(Products.Cost)
		                                FROM Products
...
SELECT dbo.AvgByType('Training apparatus') AS 'Avg cost'

--3. Пользовательская функция возвращает среднюю цену продажи по каждой дате, когда осуществлялись продажи.

CREATE FUNCTION AVGByDate()
RETURNS @resultTable TABLE ([Date] DATE NOT NULL,AvgCost INT NOT NULL)
BEGIN
...
RETURNS @resultTable TABLE (Product NVARCHAR(30) ,
                           [Type] NVARCHAR(30),
...
INSERT INTO @resultTable  SELECT TOP 1 Products.[Name],[Type].[Name],Fabricator.[Name],Sales.[Date],Products.PrimeCost,Cost,Sales.Quantity,Cost*Sales.Quantity AS 'Total sum' , Employees.FullName,Clients.FullName
                          FROM Sales
                          JOIN Storage ON Sales.StorageId = Storage.Id
                          JOIN Products ON Storage.ProductId = Products.Id
                          JOIN [Type] ON Products.TypeId = [Type].Id
...
SELECT * FROM dbo.FisrtSell()



--6. Пользовательская функция возвращает информацию о заданном виде товаров конкретного производителя. 
--Вид товара и название производителя передаются в качестве параметров.



CREATE FUNCTION GetProductByTypeAndFabricat(@Type NVARCHAR(30),@Fabricator NVARCHAR(30))
RETURNS @resultTable TABLE ([Name] NVARCHAR(30), PrimeCost INT)
BEGIN 
	INSERT INTO @resultTable  SELECT Products.[Name], PrimeCost
                              FROM Products
                              JOIN Type ON Products.TypeId = Type.Id
                              JOIN Fabricator ON Products.FabricatorId = Fabricator.Id
							  WHERE Fabricator.[Name] LIKE @Fabricator AND [Type].[Name] LIKE @Type
    RETURN 
END


SELECT * FROM dbo.GetProductByTypeAndFabricat('Protection','Adidas')


--7. Пользовательская функция возвращает информацию о покупателях, которым в этом году исполнится 45 лет.
-- возраст нету вместо этого ,те у кого скидка 19%

CREATE FUNCTION GetClietnBySkidka(@Percent INT) -- возращаит только по 19 как то узко, поэт по параметру добавил
RETURNS @resultTable TABLE (FullName NVARCHAR(30), Email NVARCHAR(30),Phone NVARCHAR(30),Gender NVARCHAR(30),IsSubcribe NVARCHAR(30),DateSubcribe NVARCHAR(30))
BEGIN
INSERT INTO @resultTable SELECT FullName,Email,Phone,Gender,IsSubcribe,DateSubcribe
                         FROM Clients
                         WHERE Clients.Discount LIKE @Percent

RETURN
END

 
 SELECT * FROM dbo.GetClietnBySkidka(19)